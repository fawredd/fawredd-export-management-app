...
import { getStorageProvider, isRunningOnVercel } from './storage.service';

/**
 * PDF Generator Service
 * Generates PDF documents for invoices and packing lists.
 * - Local: writes to disk under uploads/pdfs/
 * - Vercel: generates in-memory and uploads to blob storage
 */

// Only create the directory when running locally
const pdfsDir = path.join(process.cwd(), 'uploads/pdfs');
if (!isRunningOnVercel() && !fs.existsSync(pdfsDir)) {
  fs.mkdirSync(pdfsDir, { recursive: true });
}

/**
 * Generate a PDF into a Buffer (used for both local and Vercel).
 */
...
/**
 * Save a PDF buffer: locally to disk, or to blob storage on Vercel.
 */
async function savePdf(filename: string, buffer: Buffer): Promise<string> {
  if (isRunningOnVercel()) {
    const storage = getStorageProvider('pdfs');
    return storage.upload(`pdfs/${filename}`, buffer, 'application/pdf');
  } else {
...
export class PdfGeneratorService {
  ...
  async generateInvoice(invoiceData: any): Promise<string> {
    ...
    return savePdf(filename, buffer);
  }
